...
/*             |ID LBRACK exp RBRACK OF exp                                 {$$=A_ArrayExp(EM_tokPos, S_Symbol($1), $3, $6);}	
             ...
             |ID LPAREN ExpList LPAREN                                    {$$=A_CallExp(EM_tokPos, S_Symbol($1), $3);}
             |OP                                                          {$$=$1;}
             |lvalue ASSIGN exp                                           {$$=A_AssignExp(EM_tokPos, $1, $3);}
             |ifExp                                                       {$$=$1;}
             |WHILE exp DO exp                                            {$$=A_WhileExp(EM_tokPos, $2, $4);}
             |FOR ID ASSIGN exp TO exp DO exp                             {$$=A_ForExp(EM_tokPos, S_Symbol($2), $4, $6, $8);}
             |LET decs IN exps END                                        {$$=A_LetExp(EM_tokPos, $2, A_SeqExp(@1.first_line, $4));}
;
...
vardec:       VAR ID ASSIGN exp                                           {$$=A_VarDec(EM_tokPos, S_Symbol($2), NULL, $4);}
             |VAR ID COLON ID ASSIGN exp                                  {$$=A_VarDec(EM_tokPos, S_Symbol($2), S_Symbol($2), $6);}
;
...
OP:           exp TIMES exp                                               {$$=A_OpExp(EM_tokPos, A_timesOp, $1, $3);}
             |exp DIVIDE exp                                              {$$=A_OpExp(EM_tokPos, A_divideOp, $1, $3);}
             |MINUS exp                                                   {$$=A_OpExp(EM_tokPos, A_minusOp, A_IntExp(@1.first_line, 0), $2);}
